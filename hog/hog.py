...
def roll_dice(num_rolls, dice=six_sided):
    ...
    #I wrote my code again, but using lists
    #rolled_numbers = [dice() for i in range[0, num_rolls]]
    # score = sum(rolled_numbers)
    # if 1 in rolled_numbers:
    #     score = 1
    #return score
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    """
    # which returns the number of points scored for a turn by rolling the given dice num_rolls times.
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        running_high = 0
        last_score = 0
        if abs(last_score - score1) > running_high:
            print(last_score, "point(s)! That's a record gain for Player ", str(who) , "!")
            running_high = abs(last_score-score1)
        return say
    return say
